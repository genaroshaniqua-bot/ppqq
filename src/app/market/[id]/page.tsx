...
export default async function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!product) notFound();
  const displayCoverUrl = await resolveProductCover(supabase, product.cover_url);
  const [{ data: seller }, { data: artist }] = await Promise.all([
    supabase.from("public_profiles").select("display_name, avatar_url, bio").eq("id", product.seller_id).single(),
      ...
      <div className="mt-5 grid gap-6 lg:grid-cols-[1fr_420px]">
        <div className="space-y-6">
          <section className="overflow-hidden rounded-[32px] border border-line bg-white shadow-soft">
            <div className="relative aspect-[16/9] bg-bg">{displayCoverUrl ? <div role="img" aria-label={`${product.title} 商品封面`} className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${displayCoverUrl})` }} /> : <div className="grid h-full place-items-center text-primary"><Package size={54} aria-hidden="true" /><span className="sr-only">暂无商品封面</span></div>}<span className="absolute left-4 top-4 inline-flex items-center gap-2 rounded-pill bg-white/92 px-4 py-2 text-xs font-black shadow-soft"><DeliveryIcon size={15} />{product.kind === "physical" ? "实体配送" : "数字交付"}</span></div>
