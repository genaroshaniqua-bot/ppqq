...
export default async function PublicArtistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: portfolios }, { data: services }, { data: reviews }] = await Promise.all([
    supabase.from("portfolios").select("id, title, image_url, tags").eq("artist_id", id).order("created_at", { ascending: false }),
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="mt-8" aria-labelledby="artist-portfolio-heading"><div className="flex items-center gap-2"><Images size={20} className="text-primary" /><h2 id="artist-portfolio-heading" className="font-display text-3xl font-black">作品集</h2></div><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{portfolios?.map((item) => <article key={item.id} className="overflow-hidden rounded-[24px] border border-line bg-white shadow-soft"><div role="img" aria-label={item.title} className="aspect-[4/3] bg-bg bg-cover bg-center" style={{ backgroundImage: `url(${item.image_url})` }} /><div className="p-4"><h3 className="font-display text-xl font-black">{item.title}</h3><p className="mt-2 text-xs font-bold text-muted">{item.tags.join(" · ")}</p></div></article>)}{!portfolios?.length ? <p className="rounded-card border border-dashed border-line bg-white p-6 text-sm font-semibold text-muted">画师尚未公开作品。</p> : null}</div></section>
