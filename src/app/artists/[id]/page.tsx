...
export default async function PublicArtistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: profile }, { data: artist }] = await Promise.all([
    supabase.from("public_profiles").select("id, display_name, avatar_url, bio, created_at").eq("id", id).maybeSingle(),
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="mt-8" aria-labelledby="artist-portfolio-heading"><div className="flex items-center gap-2"><Images size={20} className="text-primary" /><h2 id="artist-portfolio-heading" className="font-display text-3xl font-black">作品集</h2></div><PublicPortfolioGallery items={displayPortfolios} /></section>
