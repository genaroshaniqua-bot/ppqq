...
import { COMMUNICATION_NOTIFICATION_TYPES, IMPORTANT_NOTIFICATION_TYPES } from "@/lib/notifications";
...
export function NotificationCenter() {
  ...
  const load = useCallback(async () => {
    const { data, error: queryError } = await createSupabaseBrowserClient()
      .from("notifications")
      .select("id, type, title, body, related_order_id, read_at, created_at")
  ...
  return <article id="notifications" className="rounded-card border border-line bg-white p-5 shadow-soft">
    ...
    {loading ? <div className="flex justify-center py-10"><LoaderCircle className="animate-spin text-primary" /></div> : <div className="mt-5 space-y-3">
      {items.map((item) => {
        const Icon = COMMUNICATION_NOTIFICATION_TYPES.has(item.type) ? MessageCircle : ReceiptText;
      ...
      {items.length === 0 ? <p className="rounded-[22px] bg-bg px-4 py-8 text-center text-sm font-semibold text-muted">暂无重要通知。新的沟通消息、报价、交付、审核、争议和订单推进会显示在这里。</p> : null}
    </div>}
